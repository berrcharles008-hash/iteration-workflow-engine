...
### 1.2.1 硬下限规则（★ 2026-09-16 新增）

> **加权总分仍是唯一判据**，硬下限只是"下限兜底"：结构性变更一旦命中，等级不得低于 🟡。

**触发条件**：以下因素命中**任一**（得分 > 0）⇒ 最终等级 **不得低于 🟡 中等**：

- `new_db_table`（新数据库表）
- `ddl_change`（DDL 变更）
- `database_logic_change`（数据库逻辑变更）
- `new_api`（新 API 接口）
- `cross_module_impact`（跨模块/跨服务影响）

**规则**：
1. **只提升不降低**：硬下限仅把低于 🟡 的结果抬升至 🟡，不干预 🟡 / 🔴 之间的判定（🔴 仍按 ≥6 分判定）。
2. **用户可覆盖**：用户可显式要求维持 🟢，但 Agent **必须告知后果**（跳过 03 交叉审查、1-5x 清单互审等 🟡 强制项），并在 `complexity_adjustments.reason` 记录 `"用户覆盖硬下限：{原因}"`。
3. **不影响典型组合**：`workflow-engine.md` 分级表的典型组合数值保持不变。

**语义回归基准**：

| 场景 | 评分 | 硬下限 | 最终 |
|------|------|:------:|:----:|
| 新表(2) + 新接口(2) + 1-2 文件(0) = 4 | 4 分（🟡） | 触发（未改变结果） | 🟡 |
| 纯文案改动，无上述因素 | 0 分（🟢） | 不触发 | 🟢 |
| 仅新接口(2) + 1-2 文件(0) = 2 | 2 分（🟢） | 触发 → 抬升 | 🟡 |

### 1.3 复杂度升降级机制

> **交叉引用**：初评（Step C，01 阶段）→ **03 强制复评 `step-3-rescore`**（见 `phase-03.md`）→ **04 实际值回填 Step 5.6**（见 `phase-04.md`），三段式记录。
